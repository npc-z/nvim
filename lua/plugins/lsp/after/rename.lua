-- lsp rename preview like inccommand
-- you're in normal mode and can do things like gUiw and everything you would expect.

-- https://github.com/saecki/dotfiles/blob/main/.config/nvim/lua/config/lsp/rename.lua
-- https://www.reddit.com/r/neovim/comments/1eott1t/lsp_rename_preview_like_inccommand/
...
function M.rename(opts)
    ...
    if editing_ranges then
        ...
        for _, range in ipairs(editing_ranges) do
            ...
            local extmark_id =
                vim.api.nvim_buf_set_extmark(M.doc_buf, extmark_ns, line, start_col, {
                    end_col = end_col,
                    virt_text_pos = "inline",
    ...
    vim.api.nvim_create_autocmd(
        { "TextChanged", "TextChangedI", "TextChangedP", "CursorMoved" },
        {
            group = group,
            buffer = M.buf,
            callback = M.update,
...
function M.submit()
    ...
    local resp = lsp_request_sync(
        M.client,
        lsp_methods.textDocument_rename,
        params,
        request_timeout,
        M.doc_buf
    )
